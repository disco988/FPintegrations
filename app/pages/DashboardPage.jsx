...
export default function DashboardPage() {
  ...
  const riskCounts = clients.reduce((acc, c) => {
      ...
      <div className="flex flex-col gap-6 max-w-5xl">
        ...
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl py-20 text-center flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400 text-sm max-w-xs">
            Add your first client to start tracking portfolios and see your
    ...
    <div className="flex flex-col gap-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <StatCard
          ...
          sub={
            plan?.clientLimit
              ? `of ${plan.clientLimit} on ${plan.name}`
              : `on ${plan?.name}`
          }
          icon="👥"
        />
        <StatCard
          label="Avg Portfolio"
          value={
            clientCount ? formatAUM(Math.round(totalAUM / clientCount)) : "—"
          }
          sub="per client"
          icon="📊"
        />
        <StatCard
          label="Dominant Risk"
          value={dominantRisk.charAt(0).toUpperCase() + dominantRisk.slice(1)}
